...
@section('page_sub_title' , 'تعديــل مادة')
@section('breadcrumb')
 ...
 <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
            <div class="max-w-xl">
                <section>
                    <form method="post" action="{{ route('admin.categories.update' , $category->id) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        @method('PATCH')
                        @if(session('success'))
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name_ar" type="text" class="mt-1 block w-full" :value="$category->name"  autofocus autocomplete="name" />
                            <x-input-error class="mt-2" :messages="$errors->get('name_ar')" />
                        </div>
                
                        <div>
                            <x-input-label for="en_name" :value="trans('form.name_en') " />
                            <x-text-input id="name" name="name_en" type="text" class="mt-1 block w-full" :value="$category->attrByLocale('en' , 'name')"  autofocus autocomplete="name" />
                            <x-input-error class="mt-2" :messages="$errors->get('name_en')" />
                        </div>
                        <div>
                            <x-input-label for="description_ar" :value="trans('form.desc_ar') " />
                            <x-text-input id="text" name="description_ar" type="text" class="mt-1 block w-full" :value="$category->description"  autocomplete="description_ar" />
                            <x-input-error class="mt-2" :messages="$errors->get('description_ar')" />
                        </div>
                        <div>
                            <x-input-label for="description_en" :value="trans('form.desc_en') " />
                            <x-text-input id="text" name="description_en" type="text" class="mt-1 block w-full" :value="$category->attrByLocale('en' , 'description')" autocomplete="description_en" />
