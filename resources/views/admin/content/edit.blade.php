 ...
 <div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
            <div class="max-w-xl">
                <section>
                    <form method="post" action="{{ route('admin.contents.update' , $content->id) }}" enctype="multipart/form-data" class="mt-6 space-y-6">
                        @csrf
                        @method('PATCH')
                        @if(session('success'))
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="title" type="text" class="mt-1 block w-full" :value="$content->title"  autofocus autocomplete="name" />
                            <x-input-error class="mt-2" :messages="$errors->get('title')" />
                        </div>
                        <div>
                            <x-input-label for="file" :value="trans('form.file') " />
                            <x-text-input id="file" name="file" type="file" class="mt-1 block w-full" :value="$content->image"  autofocus autocomplete="name" />
                            <x-input-error class="mt-2" :messages="$errors->get('file')" />
                        </div>
                        <div>
                            <x-input-label for="content" :value="trans('form.content') " />
                            <textarea id="content" name="content" type="text" class="mt-1 block w-full form-control" :value="$content->content"  autofocus autocomplete="name" cols="30" rows="10">
                                {{ $content->content }}
                            </textarea>
